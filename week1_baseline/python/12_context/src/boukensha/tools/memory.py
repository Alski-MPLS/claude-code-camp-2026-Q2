...
_PROMPT_HEADER = (
    ...
    "This is mandatory, not optional: every time you enter a new room — one you "
    "have not already recorded in world.md — you MUST call read_memory('world'), "
    "merge the new room in, and call write_memory('world', ...) with the merged "
    "result BEFORE taking any other action (before your next move, look, or "
    "anything else). Do the same for player.md whenever your character state "
    "changes (level, stats, goals). Do not wait until the end of the session or "
    "a summary to do this — by then it is too late, nothing will have been saved. "
