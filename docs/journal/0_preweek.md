...
- It looks like there is a mud_manager in the repo that already has a session abstraction built. This was not used though, the plain agent built by python scripts that were on the fly. Not sure if this was intentional or if it didn't know it was there which could be connected to the MCP question above. 
...
One of the challenges was to level up enough to defeat the minotaur in the newbie zone. Both test approaches were working toward that goal. The second experiment got the closest but then stopped when it ran into the level 7 requirement.

#### 1. Prompted Agent (CLAUDE.md + reference files)
...
#### Additional Observations with the Prompted Agent
...
- The real breakthrough for finding the bakery wasn't live exploration -- It was grepping the already-parsed world JSON files in the preview/data/world/wld folder. It gave exact room IDs and a straight forward path without wandering. This would not be available to the player... The agent cheated.... 

#### 2. Skill-Driven Agent (tbamud-explorer skill)
...
- I forgot that I have Graphify installed so I will have to make sure this is not going to add too much junk to the repo. Not an issue will add the appropriate folders in gitignore in case I decide to use it in the future.
...
#### Additional Observations with the Skill-Driven Agent
...
- Having a structured queue would help with this in the player.md or another area. Maybe have an ID, priority, status (ready/blocked/in-progress) and a blocked field.
...
- Utilize updating the plan and subplans during the session.
    ...
    - There should be checks after ever decision or observation, a plan validity check.
...
| | Prompted Agent | Skill-Driven Agent |
|---|---|---|
...
- The biggest lesson: I was probably the cause of most of the failures, not the tool. I could have made the instructions clearer, which would have fixed a lot of this before it became a problem.